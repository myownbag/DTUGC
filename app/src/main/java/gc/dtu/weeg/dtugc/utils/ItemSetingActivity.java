...
//                                ,CodeFormat.dip2px(this,400)));
                    ...
                    else if(tempaddr.equals("222"))
                    {
                        LocalSetaddr222ExtraInfoView view;
//                        byte[] setbytes = LocalSetaddr222ExtraInfoView.strinfo2bytes(setcontent);
                        view =new LocalSetaddr222ExtraInfoView(this,setcontent);
                        RelativeLayout relativeLayout = findViewById(R.id.item_activity_setting_content);
                        relativeLayout.setVisibility(View.GONE);
                        ExtraSetView.addView( view );
                    }
...
//                Log.d("zl","addrtemp.equals 210:"+CodeFormat.byteToHex(sendbuf,sendbuf.length));
            ...
            else if(addrtemp.equals("222"))
            {
                byte[] settings;
                LocalSetaddr222ExtraInfoView View222= (LocalSetaddr222ExtraInfoView) ExtraSetView.getChildAt(0);
                settings=View222.dacodeshowinfo();
                if(settings == null)
                {
                    return;
                }
